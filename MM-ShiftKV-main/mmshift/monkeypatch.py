...
def replace_qwen(method):
    ...
    Args:
        method: Patch selector such as ``shiftkv``, ``snapkv``, ``sparsemm``,
            ``adakv``, ``keydiff``, ``streamingllm``, or ``fullkv``. Unknown
            names fall back to the Full KV patch set.
    """
    ...
    if method == "streamingllm":
        ...
        print("Use Full KV")
        # Accept image_token_positions / image_token_lengths from lmms_eval,
        # then drop them in attention so the KV cache is not pruned.
        _install_qwen25_base_patchset()
